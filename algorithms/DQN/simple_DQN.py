...
class QNetwork(nn.Module):                                                                                  ### Defines a function approximator neural network that inherits from nn.Module.
    def __init__(self, state_size: int, action_size: int) -> None:                                          ## Constructor method fits input and output sizes to state and action spaces.
        """ 
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:                                                     ## Defines the forward pass method (assign Q-values to actions in this state).
        """ 
...
class DQNAgent:                                                                                             ### Defines the DQNAgent class that will act and update its network.
    def __init__(self, state_size: int, action_size: int, gamma: float = 0.99, lr: float = 0.001) -> None:  ## Constructor method for DQNAgent.
        """
    ...
    def act(self, state: list) -> int:                                                                      ## Method for selecting actions.
        """
    ...
    def train_step(self, state: list, action: int, reward: float, next_state: list, done: bool) -> None:    ## Method to train the Q-network.
        """
    ...
    def save_model(self, path: str) -> None:                                                                ## Method to save the model.
        """
            Save the Q-network model to a file.
                Args:-------path:   str, path to save the model
                Returns:----None
        """
        torch.save(self.q_network.state_dict(), path)                                                       # Saves the Q-network state dictionary to a file.
