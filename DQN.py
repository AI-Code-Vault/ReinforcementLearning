...
class QNetwork(nn.Module):                                                                  ### Defines a function approximator that inherits from nn.Module
    def __init__(self, state_size, action_size):                                            ## Constructor method
        super(QNetwork, self).__init__()                                                    # Calls parent class constructor
    ...
    def forward(self, x):                                                                   ## Defines the forward pass method
...
class DQNAgent:                                                                             ### Defines the DQNAgent class
    def __init__(self, state_size, action_size, gamma=0.99, lr=1e-3):                       ## Constructor with default parameters
        ...
        self.target_network.eval()                                                          # Sets target network to evaluation mode
    ...
    def act(self, state):                                                                   ## Method for selecting actions
        if random.random() < self.epsilon:                                                  # Checks if random number is less than epsilon
            return random.randint(0, self.action_size - 1)                                  # Returns random action
        else:
    ...
    def train_step(self, state, action, reward, next_state, done):                          ## Training method
                                                                                            # Converts inputs to PyTorch tensors and adds batch dimension
    ...
    def update_target_network(self):                                                        ## Method to update target network
        self.target_network.load_state_dict(self.q_network.state_dict())                    # Copies weights from Q-network to target network

    def decay_epsilon(self):                                                                ## Method to decay epsilon
